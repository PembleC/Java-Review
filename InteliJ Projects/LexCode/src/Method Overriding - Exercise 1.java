...
class User{
    ...
    public boolean makePayment(double billAmount){
    	if(this.walletBalance > billAmount){
    		this.walletBalance = walletBalance - billAmount;
    		return true;
    	} else {
    		return false;
    	}
...
class PremiumUser extends User{
	...
	public PremiumUser(int id, String userName, String emailId, int walletBalance) {
		super(id, userName, emailId, walletBalance);
	}

	@Override
	public boolean makePayment(double billAmount){
		double wB = getWalletBalance();
		double credit = billAmount * 0.1;
		if(wB > billAmount){
    		this.setWalletBalance(wB - billAmount);
    		rewardPoints += credit;
    		return true;
    	} else {
    		return false;
